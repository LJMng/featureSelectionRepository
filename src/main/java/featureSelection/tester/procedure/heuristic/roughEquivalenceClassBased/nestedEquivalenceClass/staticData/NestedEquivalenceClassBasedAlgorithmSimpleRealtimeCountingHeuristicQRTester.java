...
import featureSelection.basic.support.alg.roughEquivalenceClassBased.nestedEquivalenceClass.SimpleCountingNECBasedStrategy;
...
import featureSelection.tester.procedure.heuristic.roughEquivalenceClassBased.nestedEquivalenceClass.staticData.procedure.SignificantAttributeSeekingLoopProcedureContainer;
import featureSelection.tester.procedure.heuristic.roughEquivalenceClassBased.original.extension.simpleCounting.realtime.RECBasedAlgorithmSimpleRealtimeCountingHeuristicQRTester;
...
/**
 * Tester Procedure for <strong>(Simple) Nested Equivalence Classes based (S-NEC)</strong> Feature
 * Selection.
 * <p>
 * <strong>Notice</strong>:
 * This tester is implemented not for the purpose of experiments, but purely as a NEC version of S-REC.
 * It is recommended to use {@link RECBasedAlgorithmSimpleRealtimeCountingHeuristicQRTester} instead.
 * <p>
 ...
 * 		<p>Create a new instance of {@link NestedEquivalenceClassBasedRealtimeSimpleCountingCalculation}.
 * 	</li>
 * 	<li>
 * 		<strong>Initiate reduct list</strong>:
 * 		<p>Initiate a reduct list.
 * 	</li>
 * 	<li>
 ...
 * </ul>
 *
 * @see RECBasedAlgorithmSimpleRealtimeCountingHeuristicQRTester
 *
 * @see SimpleCountingNECBasedStrategy
...
public class NestedEquivalenceClassBasedAlgorithmSimpleRealtimeCountingHeuristicQRTester<Sig extends Number>
	extends SelectiveComponentsProcedureContainer<Collection<Integer>>
	implements TimeSum,
				StatisticsCalculated,
				ReportMapGenerated<String, Map<String, Object>>,
				HashSearchStrategy,
				SimpleCountingNECBasedStrategy,
				QuickReductHeuristicReductStrategy
{
	...
	public NestedEquivalenceClassBasedAlgorithmSimpleRealtimeCountingHeuristicQRTester(ProcedureParameters parameters, boolean logOn) {
	...
	public String shortName() {
		Collection<ProcedureContainer<?>> subCons = null;
		
		String corer;
		if (getComponentMap().containsKey("Get Core")) {
			subCons = this.getComponentMap().get("Get Core").getSubProcedureContainers().values();
	...
	public void initDefaultComponents(boolean logOn) {
		ProcedureComponent<?>[] componentArray = new ProcedureComponent<?>[] {
			...
			// 2. Initiate.
			new TimeCountedProcedureComponent<NestedEquivalenceClassBasedRealtimeSimpleCountingCalculation<Sig>>(
					...
					false, (component, parameters) -> {
						/* ------------------------------------------------------------------------------ */
						int p=0;
						Class<NestedEquivalenceClassBasedRealtimeSimpleCountingCalculation<Sig>> calculationClass =
								(Class<NestedEquivalenceClassBasedRealtimeSimpleCountingCalculation<Sig>>)
								parameters[p++];
						/* ------------------------------------------------------------------------------ */
						TimerUtils.timeStart((TimeCounted) component);
						/* ------------------------------------------------------------------------------ */
						return calculationClass.newInstance();
					}, 
			...
			new TimeCountedProcedureComponent<Collection<Integer>>(
					...
					false, (component, parameters) -> {
						...
						TimerUtils.timeStart((TimeCounted) component);
						/* ------------------------------------------------------------------------------ */
						return new HashSet<>(attributes.length);
					}, 
					(component, red) -> {
						/* ------------------------------------------------------------------------------ */
				...
				}.setDescription("Initiate reduct list"),
			// 4. Sig loop.
