 ...
 * <p>
 * >> "best fitness == max fitness" <strong>NOT</strong> guaranteed. Not equal if exit through reaching
 * max convergence.
 * <p>
 ...
 * 		<p>Only the global best fitness with current best fitness value and less attributes will be
 * 	    	preserved. Global best fitnesses with the same fitness value as current global best fitness
 * 	    	value with less/equal attribute number will all be deserted. Finally, when <strong>exiting
 * 	    	loop(i.e. reaching max convergence or max iteration)</strong>, only <strong>1 global best
 * 	    	fitness</strong> which was preserved with the global best fitness value and less attributes
 * 	    	will be preserved and returned.
 *  	<p>
 ...
 * </ul>
 * <p>
 * In this {@link DefaultGenerationLoopProcedureContainer}(only!), the following parameters are used in 
 * {@link #getParameters()}:
 * <ul>
	...
	extends DefaultGenerationLoopProcedureContainer<Cal, Sig, CollectionItem, Hrmny, FValue>
{
	...
	public String shortName() {
		return "IHS Generation loops(Global best prioritized)";
	}
		...
		return new ProcedureComponent<?>[] {
					...
					(component, result) -> {
						...
						if (logOn) {
							...
							log.info(LoggerUtil.spaceFormat(2, "Global best fitness: {}"), 
									bestFitness.getFitnessValue()
							);
							log.info(LoggerUtil.spaceFormat(2, "Max best fitness: {}"), 
									params.getMaxFitness());
						}
						/* ------------------------------------------------------------------------------ */
						// Statistics
			...
			new TimeCountedProcedureComponent<Boolean>(
					...
					false, (component, parameters) -> {
						...
						int bestFitnessCmp = params.getRedAlg()
													.compareToBestFitness(generRecord, bestFitness);
						// not the best, skip
						if (bestFitnessCmp<0) {
							// do nothong...
						...
						// 1 of the bests
							...
							// if feature subset is shorter
							if (gBest.getAttributes().length!=0 && 
								gBest.getAttributes().length<generRecord.getDistinctAttributes().iterator().next().key().length
							) {
								generRecord.updateBestFitness(bestFitness, gBest);
								generRecord.resetConvergence();
