 ...
 * 		if solutions in {@link GenerationRecord#getGlobalBestFitnessCollection()} exceeds, select
 * 		randomly and remove the rest.
 * </li>
 * </ul>
 * 
 * @author Benjamin_L
 */
@Slf4j
public class RandomThresholdBasedMultiSolutionSelectionProcedureContainer4NEC<Velocity,
				...
				ReportMapGenerated<String, Map<String, Object>>
{
	...
	public RandomThresholdBasedMultiSolutionSelectionProcedureContainer4NEC(ProcedureParameters parameters, boolean logOn) {
			...
			new TimeCountedProcedureComponent<GenerationRecord<Velocity, Posi, FValue>>(
					...
					false, (component, parameters) -> {
						...
						if (geneRecord.getGlobalBestFitnessCollection().size()>limit) {
							Collection<Integer> stays = 
								RandomUtils.randomUniqueInts(
									0, geneRecord.getGlobalBestFitnessCollection().size(), limit, 
									random
								);
							Collection<Fitness<Posi, FValue>> globalBestFitnessCollection =
									geneRecord.getGlobalBestFitnessCollection();
							Iterator<Fitness<Posi, FValue>> itr = globalBestFitnessCollection.iterator();
							for (int i=0; itr.hasNext(); i++) {
								itr.next();
								if (!stays.contains(i))	itr.remove();
								if (limit>=globalBestFitnessCollection.size())	break;
							}
						}
						/* ------------------------------------------------------------------------------ */
						return geneRecord;
