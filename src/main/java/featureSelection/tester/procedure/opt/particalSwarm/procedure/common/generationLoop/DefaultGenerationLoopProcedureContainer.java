 ...
 * max convergence.
 * <p>
 * Mainly, this contains 6 ProcedureComponents : 
 * <ul>
 * 	<li>
 ...
 * 	 <strong>reaching max convergence</strong>, break the loop. Otherwise, loop until <strong>reaching 
 * 	 max iteration or reaching max distinct max fitness count(only if max fitness is satisfied)</strong>.
 * 	 <p>
 ...
 * 		<p>Update particles' individual best fitness, i.e. if the new fitness happens to be better, update the 
 * 			particle with the new one and the position it represents.
 * 	</li>
 * 	<li>
				...
				ReportMapGenerated<String, Map<String, Object>>
{
	...
	public DefaultGenerationLoopProcedureContainer(ProcedureParameters paramaters, boolean logOn) {
							...
							exit =	// if any is satisfied
									...
									(params.getMaxDistinctBestFitness()==null? 
										...
										(params.getReductionAlgorithm().compareMaxFitness(params.getMaxFitness(), geneRecord)<=0 &&
										 geneRecord.countDistinctBestFitness() >= params.getMaxDistinctBestFitness())
										)
									);
						}while(!exit);
						return geneRecord;
