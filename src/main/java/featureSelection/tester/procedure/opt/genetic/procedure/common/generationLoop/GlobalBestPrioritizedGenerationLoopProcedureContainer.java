 ...
 * <p>
 * Mainly, this {@link ProcedureContainer} contains {@link ProcedureComponent}s:
 * <ul>
 *  <li>
 *  	<strong>Generation loop procedure controller</strong>:
 *  	<p>Different from {@link DefaultGenerationLoopProcedureContainer},
 *  	<p>Only the global best fitness with current best fitness value and attribute sets with the
 *      	least size would be preserved. Global best fitnesses with the same fitness value as current
 *      	global best fitness value with less/same attribute number are deserted. Finally,
 *      	when <strong>exiting loop(i.e. reaching max convergence or max iteration)</strong>, only
 *      	<strong>1 global best fitness</strong> which was preserved with the global best fitness
 *      	value and the attribute set with least attribute number is returned.
 *      <p>
 ...
 *  	<p>Global best fitness is updated if and only if the fitness is greater. (Always preserved
 *      	only 1 global best fitness record)
 *  </li>
 *  <li>
...
{
	public GlobalBestPrioritizedGenerationLoopProcedureContainer(ProcedureParameters paramaters, boolean logOn) {
		super(paramaters, logOn);
	}
		...
		return new ProcedureComponent<?>[] {
			...
			new TimeCountedProcedureComponent<Object[]>(
						...
						for (int i=0; i<chromosomeFitness.length; i++) {
							...
							if (geneRecord.getBestFitness()==null) {
								...
								// 1 of the bests
									...
									// if feature subset is smaller in size
									if (chromosomeFitness[i].getChromosome().getAttributes().length <
										geneRecord.getFitness().iterator().next().getChromosome().getAttributes().length
									) {
										// update best fitness
										geneRecord.updateBestFitness(chromosomeFitness[i]);
										resetConvergenceCount = true;
					...
					(component, result) -> {
						...
						if (logOn) {
							...
							log.info(LoggerUtil.spaceFormat(2, "x{} chromosome(s)"), geneRecord.getFitness().size());
						}
						/* ------------------------------------------------------------------------------ */
						// Statistics
						//	[STATISTIC_ITERATION_INFOS]
			...
			new TimeCountedProcedureComponent<Boolean>(
					...
					false, (component, parameters) -> {
						...
						ReductionParameters<Cal, Sig, CollectionItem, Chr, FValue> params = 
								(ReductionParameters<Cal, Sig, CollectionItem, Chr, FValue>)
								parameters[p++];
						boolean resetConvergenceCount = (boolean) parameters[p++];
						/* ------------------------------------------------------------------------------ */
						TimerUtils.timeStart((TimeCounted) component);
						/* ------------------------------------------------------------------------------ */
			...
			new TimeCountedProcedureComponent<Chr[]>(
					...
					false, (component, parameters) -> {
						/* ------------------------------------------------------------------------------ */
						int p=0;
						Chr[] chromosome = (Chr[]) parameters[p++];
						Fitness<Chr, FValue>[] chromosomeFitness = (Fitness<Chr, FValue>[]) parameters[p++];
