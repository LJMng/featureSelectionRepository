...
public class ToleranceClassObtainerByDirectCollectNCache 
	implements ToleranceClassObtainer 
{
	...
	public Map<Instance, Collection<Instance>> obtain(
		...
		final Map<IntArrayKey, Collection<Instance>> toleranceCache = new HashMap<>(instances.size());
		
		for (Instance ins: instances) {
			IntArrayKey key = new IntArrayKey(Instance.attributeValuesOf(ins, attributes));
			final Collection<Instance> cache = toleranceCache.get(key);
			
			if (cache!=null) {
				// use cache
				if (!cache.isEmpty())	tolerances.put(ins, new ArrayList<>(cache));
			}else {
				...
				// save cache
				toleranceCache.put(key, tolerance);
			}
		}
		return tolerances;
	}
